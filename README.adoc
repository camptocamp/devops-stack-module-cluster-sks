...
// TODO Consider creating a proper nodepool for the Longhorn storage nodes
// TODO If doing previous todo, consider changing the upgrade procedure below to talk about the Longhorn volumes
...
|[[input_cni]] <<input_cni,cni>>
|Specify which CNI to use by default. Accepted values are `calico` or `cilium`, but you cannot change this value after the first deployment. This module creates the required security group rules.
|`string`
|`"cilium"`
|no
